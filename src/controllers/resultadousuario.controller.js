...
exports.obtenerHistorialEmocional = async (req, res) => {
  try {
    const id_usuario = req.user.id; // desde el token
    const historial = await service.obtenerHistorialEmocional(id_usuario);
    res.json(historial);
  } catch (error) {
...
exports.obtenerMiTipoEmocional = async (req, res) => {
  try {
    ...
    const data = await service.obtenerMiTipoEmocional(id_u);
    res.json(data);
  } catch (err) {
